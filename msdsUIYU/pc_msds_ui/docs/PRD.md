...
### 2.1 产品名称与定位
- **产品名称**: MSDS 管理文件系统
- **产品定位**: 一款专为实验室科研机构设计的，用于实现化学品安全技术说明书 (MSDS) 电子化、集中化管理与便捷查阅的PC端应用软件。
...
### 2.4 目标平台列表
- **PC 端桌面应用程序**: Windows, macOS
- **Web 浏览器访问**: (作为扩展或未来版本考虑，初期聚焦PC客户端)

### 2.5 产品核心假设
- 实验室科研机构普遍存在MSDS管理效率低下、查阅不便的问题。
- 科研人员对快速、准确获取MSDS信息有强烈需求。
- 电子化、智能化的MSDS管理系统能显著提升实验室的安全管理水平和科研效率。
...
1.  **场景一: 科研人员实验前查阅MSDS**
    ...
    *   **需求**: 他需要详细了解丙酮的闪点、爆炸极限、毒性、所需个人防护装备（如手套、护目镜类型）、以及万一发生泄漏或皮肤接触时的应急处理措施。
    *   **当前做法 (痛点)**: 可能需要去药品柜查找纸质MSDS，或者在网上搜索，信息来源不一，耗时且不确定是否为最新版本。
    *   **期望系统支持**: 打开PC端MSDS管理系统，在搜索框输入“丙酮”或其CAS号，系统快速列出匹配的MSDS。点击查看，在线预览MSDS全文，特别是关注“危险性概述”、“急救措施”、“消防措施”、“泄漏应急处理”和“操作处置与储存”等章节。
...
2.  **场景二: 实验室管理员更新与分发MSDS**
    ...
    *   **期望系统支持**: 登录MSDS管理系统，进入管理模块。选择“上传MSDS”功能，批量或单个上传新的PDF文件。系统能够智能识别或允许手动关联化学品名称/CAS号。如果系统中已存在该化学品的旧版MSDS，系统提示更新并自动归档旧版本。王老师还可以选择向特定用户组发送更新通知。

3.  **场景三: 发生化学品意外泄漏时的应急响应**
    *   **用户**: 任何在场的实验室人员
    *   **情景**: 实验过程中，一个装有“浓硫酸”的试剂瓶意外打翻，少量液体溅出。
    *   **需求**: 现场人员需要立即知道浓硫酸的腐蚀性、正确的泄漏处理方法（如用何种吸收材料）、以及皮肤接触后的急救措施。
    *   **当前做法 (痛点)**: 慌乱中可能找不到纸质MSDS，或者纸质版信息不全、字迹不清。
    *   **期望系统支持**: 任何一台联网的电脑上，快速打开MSDS系统（或有应急查阅入口），输入“浓硫酸”，立即获取其MSDS。系统应高亮显示或能快速定位到“泄漏应急处理”和“急救措施”部分，指导现场人员正确、安全地进行处置。
...
-   **离线访问需求**: 在网络条件不佳或断网情况下，PC客户端应能访问已同步到本地的MSDS数据。
...
-   **数据集成**: MSDS管理系统有与LIMS、库存管理、采购系统等集成的趋势，形成一体化实验室管理平台。
...
-   **PC客户端优先**: 满足科研人员在实验室内稳定、高效操作的需求，同时考虑未来Web化扩展。
-   **易用性与性价比**: 提供简洁直观的操作界面，降低学习成本；制定有竞争力的价格策略，特别是针对预算有限的高校和中小型科研机构。
-   **本地化服务与支持**: 提供中文界面和及时的本地技术支持。
-   **数据安全与私有化部署**: 允许机构自主管理MSDS数据，提供私有化部署选项，解决数据敏感性问题。
...
##### 5.2.2.2 MSDS 在线预览与下载
-   **用户故事**: 作为科研人员，我想要在搜索到MSDS后，能够直接在线预览其内容，并支持在预览界面进行文本查找；同时，我也希望能将MSDS文件下载到本地，以便离线查阅或打印。
-   **用户价值**: 方便快捷地查阅MSDS详细信息，满足不同场景下的使用需求。
-   **功能逻辑与规则**: 
    -   在线预览器应支持常见的MSDS文件格式（PDF为主，DOCX, TXT等）。
    -   预览器应保持文档原始排版，支持缩放、翻页、内容搜索等基本功能。
    -   下载功能应提供原始文件。
    -   记录MSDS的查阅和下载行为（用于审计和统计）。
-   **交互要求**: 流畅的文档加载和预览体验，预览工具栏功能明确。
-   **数据需求**: MSDS文件本身。
-   **技术依赖**: 前端文档预览组件 (如 pdf.js, Mammoth.js)。
-   **验收标准**:
    1.  能成功在线预览PDF和DOCX格式的MSDS文件。
    2.  预览界面支持放大、缩小、按页码跳转功能。
    3.  在预览的PDF文档中，能成功搜索并高亮显示关键词。
    4.  能成功下载MSDS的原始文件到本地。
...
-   **响应式设计考虑**: 虽然初期聚焦PC客户端，但设计时可为未来Web化和响应式布局预留一定灵活性。
...
### 7.1 性能需求
-   **搜索响应时间**: 对于10万条MSDS数据量级，95%的关键词搜索响应时间应 < 2秒。
-   **文档预览加载时间**: 5MB以内的PDF文件，首次加载时间应 < 3秒，后续翻页流畅。
-   **并发用户数**: 系统应能支持至少50个并发用户同时进行MSDS查阅操作，10个并发用户进行管理操作。
-   **批量导入性能**: 导入1000个MSDS文件（平均大小1MB）的时间应 < 30分钟。
-   **系统启动时间**: PC客户端冷启动时间 < 10秒。
...
-   **可访问性**: 
    ...
    -   为图片和非文本内容提供替代文本（如适用）。
-   **兼容性**: 
    -   PC客户端支持 Windows 10及以上版本，macOS 最新两个大版本。
    -   (若有Web界面) 支持主流浏览器最新版本 (Chrome, Firefox, Edge, Safari)。
...
### 8.1 技术栈建议
-   **PC客户端**: 
    -   **方案一 (跨平台)**: Electron + (React/Vue/Angular) + Node.js
    -   **方案二 (原生)**: Windows (WPF/WinUI), macOS (Swift/Objective-C) - 开发成本较高，但性能和体验更优。
    -   **推荐**: 初期可考虑Electron以快速实现跨平台，若对性能和原生体验有极致要求，再考虑原生开发。
-   **后端服务**: 
    -   Java (Spring Boot)
    -   Python (Django/Flask)
    -   Node.js (Express/NestJS)
    -   Go
    -   选择依据: 团队熟悉度、生态系统、性能需求、开发效率。
-   **数据库**: 
    -   关系型数据库 (RDBMS): PostgreSQL, MySQL - 适合结构化数据和事务一致性。
    -   NoSQL数据库: MongoDB (用于存储MSDS文档本身或非结构化元数据，或与全文搜索引擎配合使用)。
    -   **推荐**: 初期可选用PostgreSQL或MySQL，若未来有大量非结构化数据或需要高性能全文检索，可考虑引入MongoDB及Elasticsearch。
-   **全文搜索引擎**: Elasticsearch, Apache Solr (若需要强大的全文检索功能)。
-   **文件存储**: 
    -   本地文件系统 (初期简单方案)。
    -   分布式文件系统 (如 MinIO, Ceph) 或云存储服务 (如 AWS S3, Azure Blob Storage) - 扩展性更好。
-   **消息队列**: RabbitMQ, Kafka (用于异步任务处理，如MSDS上传后的后台处理、通知发送等，可选)。
...
### 8.3 技术依赖与约束
-   **操作系统兼容性**: PC客户端需兼容主流Windows和macOS版本。
-   **第三方库/组件许可**: 使用的开源库或商业组件需注意其许可协议，避免法律风险。
-   **性能与资源限制**: 需考虑目标用户机构的硬件环境，避免对资源要求过高。

### 8.4 数据模型建议 (关键实体)
-   **Chemical (化学品)**: `ChemicalID (PK)`, `NameCN`, `NameEN`, `CASNumber (Unique)`, `MolecularFormula`, `Synonyms`, `SupplierID (FK)`, `Manufacturer`, `GHSClassification`, ...
-   **MSDS_Document**: `MSDSDocID (PK)`, `ChemicalID (FK)`, `Version`, `EffectiveDate`, `ExpiryDate`, `UploadDate`, `UploaderUserID (FK)`, `FilePath`, `FileSize`, `FileType`, `Language`, `Status (Current, Archived)`, ...
-   **User**: `UserID (PK)`, `Username (Unique)`, `PasswordHash`, `FullName`, `Email`, `Department`, `RoleID (FK)`, `IsActive`, ...
-   **Role**: `RoleID (PK)`, `RoleName (Unique)`, `Description`, ...
-   **Permission**: `PermissionID (PK)`, `PermissionName (Unique)`, `Description`, ...
-   **Role_Permission_Mapping**: `RoleID (FK)`, `PermissionID (FK)`
-   **User_Group**: `GroupID (PK)`, `GroupName`, ...
-   **User_Group_Membership**: `UserID (FK)`, `GroupID (FK)`
-   **Audit_Log**: `LogID (PK)`, `UserID (FK)`, `Timestamp`, `ActionType`, `TargetEntity`, `TargetEntityID`, `Details`, `IPAddress`, ...
-   **Custom_Field_Definition**: `FieldID (PK)`, `FieldName`, `FieldType`, `AppliesToEntity`, ...
-   **Custom_Field_Value**: `ValueID (PK)`, `EntityID`, `FieldID (FK)`, `FieldValue`, ...
