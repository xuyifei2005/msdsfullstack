...
const JobLogTableList: React.FC = () => {
  ...
  // 修正：valueEnum 与查询参数初始值应为对象
  const [jobGroupOptions, setJobGroupOptions] = useState<Record<string, any>>({});
  ...
  // 将重定向与 getJob 请求移动到副作用中，避免在渲染阶段触发副作用
  useEffect(() => {
    if (id === undefined) {
