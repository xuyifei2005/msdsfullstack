  ...
  useEffect(() => {
    getCacheInfo().then((res) => {
      if (res.code === 200) {
        ...
        const data: { x: string; y: number }[] = res.data.commandStats.map((item: any) => {
          return {
            x: item.name,
            y: Number(item.value),
          };
      ...
      <Row gutter={[24, 24]}>
        ...
        <Col span={12}>
          <Card title="内存信息" className={styles.card}>
            <Gauge
              height={320}
              percent={memUsage as number}
              range={{
                color: ['#C3F71F', '#B5E61D', '#FFC90E', '#FF7F27', '#FF2518'],
              }}
              statistic={{
                title: {
                  formatter: () => memUsageTitle,
                  style: {
                    fontSize: '14px',
                    color: '#666',
                  },
                ...
                content: {
                  formatter: () => `${(memUsage as number * 100).toFixed(2)}%`,
                  style: {
                    fontSize: '24px',
                    fontWeight: 'bold',
                  },
