...
def debug_extract_field():
    ...
    # 手动测试_extract_field_value_enhanced方法
    print("测试_extract_field_value_enhanced方法:")
    print("-" * 40)
    
    field_names = ["危险性类别", "危险类别"]
    result = xml_gen._extract_field_value_enhanced(hazard_content, field_names)
    ...
    for i, pattern in enumerate(patterns):
        ...
        if match:
            ...
            # 检查过滤条件
            if value and len(value) > 1 and not value.startswith('第'):
                print(f"  通过过滤条件")
            else:
                print(f"  被过滤条件拒绝")
        else:
