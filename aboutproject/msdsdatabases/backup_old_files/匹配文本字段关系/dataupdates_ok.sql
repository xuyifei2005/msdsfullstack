...
-- 手动添加了一个备注，下次关注下导入导出！
react-ui\src\locales\zh-CN\msds.ts
'msds.main.remark': '备注',
...
CREATE TABLE IF NOT EXISTS msds_hazard (
    ...
    hazard_category VARCHAR(255) COMMENT '危险性类别', -- 危险性类别
    exposure_routes VARCHAR(255) COMMENT '侵入途径', -- 侵入途径
    health_hazards TEXT COMMENT '健康危害', -- 健康危害
    environmental_hazards TEXT COMMENT '环境危害', -- 环境危害
    fire_explosion_hazards TEXT COMMENT '燃爆危险', -- 燃爆危险
...
CREATE TABLE IF NOT EXISTS msds_component (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    component_name VARCHAR(255) NOT NULL COMMENT '成分名称',-------成分名称
    component_english_name VARCHAR(255) COMMENT '成分英文名',
    component_content VARCHAR(100) COMMENT '成分含量/浓度',-------含量
...
CREATE TABLE IF NOT EXISTS msds_first_aid (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    skin_contact TEXT COMMENT '皮肤接触处理措施',-------皮肤接触处理措施
    eye_contact TEXT COMMENT '眼睛接触处理措施',-------眼睛接触处理措施
    inhalation TEXT COMMENT '吸入处理措施',-------吸入处理措施
    ingestion TEXT COMMENT '食入处理措施',-------食入处理措施
...
CREATE TABLE IF NOT EXISTS msds_fire_fighting (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    hazard_characteristics TEXT COMMENT '危险特性',-------危险特性
    harmful_combustion_products TEXT COMMENT '有害燃烧产物',-------有害燃烧产物
    suitable_extinguishing_media TEXT COMMENT '适宜的灭火介质',-------灭火方法
    ...
    fire_risk_classification VARCHAR(50) COMMENT '建规火险分级',  -- 建规火险分级
    FOREIGN KEY (msds_id) REFERENCES msds_main(id) ON DELETE CASCADE,
    UNIQUE KEY uk_msds_id (msds_id)
...
CREATE TABLE IF NOT EXISTS msds_leak_response (
    ...
    emergency_procedures TEXT COMMENT '应急处理程序',----应急处理
...
-- 7. 操作处置与储存表[7.操作处置与储存]
CREATE TABLE IF NOT EXISTS msds_handling_storage (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    handling_precautions TEXT COMMENT '操作注意事项',-------操作注意事项
    storage_precautions TEXT COMMENT '储存注意事项',-------储存注意事项
...
-- 8. 接触控制/个体防护表[8.接触控制/个体防护]
CREATE TABLE IF NOT EXISTS msds_exposure_control (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    occupational_exposure_limit TEXT COMMENT '职业接触限值',---接触限值
    china_mac VARCHAR(50) COMMENT '中国MAC值(mg/m³)',------中国MAC(mg/m3)
    usa_tlv_twa VARCHAR(50) COMMENT '美国TLV-TWA值(mg/m³)',
    usa_tlv_stel VARCHAR(50) COMMENT '美国TLV-STEL值(mg/m³)',
    former_soviet_mac VARCHAR(50) COMMENT '前苏联MAC值(mg/m³)', -- 前苏联MAC
    tlv_tn VARCHAR(50) COMMENT 'TLV-TN值(mg/m³)', -- TLV-TN
    tlv_wn VARCHAR(50) COMMENT 'TLV-WN值(mg/m³)', -- TLV-WN
    monitoring_method TEXT COMMENT '监测方法',----监测方法
    engineering_controls TEXT COMMENT '工程控制措施',------工程控制
    respiratory_protection TEXT COMMENT '呼吸系统防护',-----呼吸系统防护
    eye_protection TEXT COMMENT '眼睛防护',-------眼睛防护
    body_protection TEXT COMMENT '身体防护',-----身体防护
    hand_protection TEXT COMMENT '手部防护',-----手防护
    other_protection TEXT COMMENT '其他防护措施',------其他防护
...
CREATE TABLE IF NOT EXISTS msds_physical_chemical (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    appearance VARCHAR(255) COMMENT '外观与性状',--外观与性状
    odor VARCHAR(100) COMMENT '气味',--气味
    odor_threshold VARCHAR(50) COMMENT '气味阈值',--气味阈值
    melting_point VARCHAR(50) COMMENT '熔点(℃)',--熔点(°C)
    boiling_point VARCHAR(50) COMMENT '沸点(℃)',--沸点(°C)
    relative_density VARCHAR(50) COMMENT '相对密度(水=1)',--相对密度(水=1)
    vapor_density VARCHAR(50) COMMENT '蒸气密度(空气=1)',--相对蒸气密度(空气=1)
    vapor_pressure VARCHAR(50) COMMENT '蒸气压(kPa)',--饱和蒸气压(kPa)
    vapor_pressure_temp VARCHAR(20) COMMENT '蒸气压测定温度(℃)',
    solubility TEXT COMMENT '溶解性',--溶解性
    water_solubility VARCHAR(100) COMMENT '水中溶解度',
    pH_value VARCHAR(20) COMMENT 'pH值',--pH
    ph_concentration VARCHAR(50) COMMENT 'pH值浓度条件',
    flash_point VARCHAR(50) COMMENT '闪点(℃)',--闪点(°C)
    ignition_temperature VARCHAR(50) COMMENT '引燃温度(℃)',--引燃温度(°C)
    explosive_limit_lower VARCHAR(50) COMMENT '爆炸下限(%)',--爆炸下限%(V/V)
    explosive_limit_upper VARCHAR(50) COMMENT '爆炸上限(%)',--爆炸上限%(V/V)
    viscosity VARCHAR(50) COMMENT '粘度',
    partition_coefficient VARCHAR(50) COMMENT '分配系数(正辛醇/水)',--辛醇/水分配系数的对数值
    decomposition_temperature VARCHAR(50) COMMENT '分解温度(℃)',
    molecular_formula VARCHAR(100) COMMENT '分子式',--分子式
    main_components TEXT COMMENT '主要成分',--主要成分
    critical_temperature VARCHAR(50) COMMENT '临界温度(℃)',--临界温度(°C)
    autoignition_temperature VARCHAR(50) COMMENT '自燃温度',--自燃温度
    flammability VARCHAR(50) COMMENT '燃烧性',--燃烧性
    molecular_weight VARCHAR(50) COMMENT '分子量',--分子量
    heat_of_combustion VARCHAR(50) COMMENT '燃烧热(kJ/mol)',--燃烧热(kJ/mol)
    critical_pressure VARCHAR(50) COMMENT '临界压力(MPa)',--临界压力(MPa)
    main_usage TEXT COMMENT '主要用途',--主要用途
    other_properties TEXT COMMENT '其它理化性质',--其它理化性质
    FOREIGN KEY (msds_id) REFERENCES msds_main(id) ON DELETE CASCADE,
    UNIQUE KEY uk_msds_id (msds_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='理化特性表';

-- 10. 稳定性和反应性表[10.稳定性和反应活性]
CREATE TABLE IF NOT EXISTS msds_stability_reactivity (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    stability TEXT COMMENT '稳定性',--稳定性
    reactivity TEXT COMMENT '反应性',
    incompatible_substances TEXT COMMENT '禁配物',--禁配物
    conditions_to_avoid TEXT COMMENT '避免接触的条件',--避免接触的条件
    hazardous_reactions TEXT COMMENT '可能的危险反应',
    polymerization_hazard TEXT COMMENT '聚合危害',--聚合危害
    polymerization_conditions TEXT COMMENT '聚合反应条件',
    decomposition_products TEXT COMMENT '分解产物',--分解产物
...
CREATE TABLE IF NOT EXISTS msds_toxicological (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    acute_toxicity TEXT COMMENT '急性毒性',--急性毒性
    ...
    subacute_chronic TEXT COMMENT '亚急性和慢性毒性',--亚急性和慢性毒性
    skin_irritation TEXT COMMENT '皮肤刺激性',--刺激性
    eye_irritation TEXT COMMENT '眼睛刺激性',--刺激性
    respiratory_irritation TEXT COMMENT '呼吸道刺激性',--刺激性
    sensitization TEXT COMMENT '致敏性',--致敏性
    mutagenicity TEXT COMMENT '致突变性',--致突变性
    teratogenicity TEXT COMMENT '致畸性',--致畸性
    reproductive_toxicity TEXT COMMENT '生殖毒性',
    carcinogenicity TEXT COMMENT '致癌性',--致癌性
    ...
    rtecs VARCHAR(100) COMMENT 'RTECS编号',--RTECS
    FOREIGN KEY (msds_id) REFERENCES msds_main(id) ON DELETE CASCADE,
    UNIQUE KEY uk_msds_id (msds_id)
...
CREATE TABLE IF NOT EXISTS msds_ecological (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    ecological_toxicity TEXT COMMENT '生态毒性',--生态毒理毒性
    ...
    biodegradability TEXT COMMENT '生物降解性',--生物降解性
    biodegradation_rate VARCHAR(50) COMMENT '生物降解速率',
    non_biodegradability TEXT COMMENT '非生物降解性',--非生物降解性
    photodegradation TEXT COMMENT '光降解',
    hydrolysis TEXT COMMENT '水解',
    bioaccumulation TEXT COMMENT '生物富集或生物积累性',--生物富集或生物积累性
    bioconcentration_factor VARCHAR(50) COMMENT '生物富集因子',
    mobility_in_soil TEXT COMMENT '土壤中迁移性',
    other_environmental_effects TEXT COMMENT '其它有害作用',--其它有害作用
...
CREATE TABLE IF NOT EXISTS msds_disposal (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    waste_properties TEXT COMMENT '废弃物性质', -- 废弃物性质
    disposal_method TEXT COMMENT '废弃处置方法', -- 废弃处置方法
    disposal_precautions TEXT COMMENT '废弃注意事项', -- 废弃注意事项
...
CREATE TABLE IF NOT EXISTS msds_transportation (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    msds_id BIGINT NOT NULL COMMENT '关联MSDS主表ID',
    dangerous_goods_number VARCHAR(50) COMMENT '危险货物编号',--危险货物编号
    un_number VARCHAR(50) COMMENT 'UN编号',--UN编号
    proper_shipping_name VARCHAR(255) COMMENT '正确运输名称',
    transport_hazard_class VARCHAR(50) COMMENT '运输危险类别',
    packing_group VARCHAR(10) COMMENT '包装类别',--包装类别
    packaging_marks TEXT COMMENT '包装标志',--包装标志
    packaging_method TEXT COMMENT '包装方法',--包装方法
    marine_pollutant TINYINT(1) DEFAULT 0 COMMENT '海洋污染物(1:是,0:否)',
    transport_in_bulk TEXT COMMENT '散装运输要求',
    transportation_precautions TEXT COMMENT '运输注意事项',--运输注意事项
    emergency_response_guide VARCHAR(50) COMMENT '应急响应指南编号',
    imdg_rule_page VARCHAR(50) COMMENT 'IMDG规则页码',--IMDG规则页码字段
    FOREIGN KEY (msds_id) REFERENCES msds_main(id) ON DELETE CASCADE,
    UNIQUE KEY uk_msds_id (msds_id),
...
CREATE TABLE IF NOT EXISTS msds_regulatory (
    ...
    domestic_regulations TEXT COMMENT '国内法规', --法规信息
...
-- 创建完成提示
-- =============================================
SELECT 'MSDS管理系统数据库创建完成！' as message,
